...
function [v0, DelPepi, GGphasic, GGtonic, GGpeak, Ti_v0, Te_v0, Ttot_v0, VT_v0, CPAPmin] = Vactivebaseline(Fs, Time, Vdot, Pmask, stage, evts, Pepi, TimeGG, GG)
%Function to calculate ventilation pre-Vactive drop
%Brad Edwards, PhD
%September 2014
...
title('Select start of pre-drop ventilation');
[x,y] = ginput(1);
tdown = x(1);
clear x y

subplot(numplots,1,1)
title('Select end of pre-drop ventilation');
[x,y] = ginput(1);
tup = x(1);
clear x y;
...
VdotE(a) = 0;
VdotE = VdotE';
%Vdrive = ((Veupnea-VdotE)*LG)+Veupnea;
clear n;
...
text(double(Time(indVI(aind)))-1,mean(VdotE)+1, strcat('VE predrop =', num2str(v0,3)))
ylabel('VdotE (L/min)')
